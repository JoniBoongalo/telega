...
def categories_keyboard():
    ...
    '''keyboard = [ # Так выглядит эта переменная 
...
def start(update, context):
    ...
    if user.id in [1137442897, 890981069, 5260146834]:
        update.message.reply_text(
            f"Привет {first_name}, приятно познакомиться с тобой!"
        )
...
def stop(update: Update, context) -> int:
...
def calculation(update, context):
    ...
    all_cost = cost_calculation_for_calculation(list_data_objects)
    ...
    update.message.reply_text(f"Расход за {dayz} дней составил:\n\n{''.join(show_objects)}Что суммарно выходит {summ} лари.",
...
def days_list_generator_for_calc(days_num):
    ...
    for delta_day in range(days_num):
...
def cost_calculation_for_calculation(new_list):
